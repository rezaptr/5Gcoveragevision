...
(function () {
  ...
  function generateFixedIntervalPoints(coords, intervalM) {
    ...
    const points = [{ lat: coords[0].lat, lng: coords[0].lng }];
    let d = intervalM;
    ...
    const distToLast = haversineDistance(
      points[points.length-1].lat, points[points.length-1].lng, lastPt.lat, lastPt.lng
    );
    if (distToLast > intervalM * 0.3) points.push(lastPt);
    return points;
  }
  ...
  function saveSnapshot(siteId, routeHash, points) {
    try {
      ...
      const data = { version:'4.5', seed:FIXED_SEED, siteId, routeHash,
        createdAt: new Date().toISOString(), count: points.length, points };
      localStorage.setItem(key, JSON.stringify(data));
      console.log(`[Snapshot] 💾 Disimpan: ${points.length} titik | key: ${key}`);
      return true;
    } catch(e) { console.warn('[Snapshot] Gagal simpan:', e.message); return false; }
  }

  function loadSnapshot(siteId, routeHash) {
    try {
      const raw  = localStorage.getItem(getSnapshotKey(siteId, routeHash));
      if (!raw) return null;
      const data = JSON.parse(raw);
      if (!data.points?.length) return null;
      console.log(`[Snapshot] ✅ Loaded: ${data.points.length} titik | dibuat: ${data.createdAt}`);
      return data.points;
    } catch(e) { console.warn('[Snapshot] Gagal load:', e.message); return null; }
  }
  ...
  function initAzimuthInputs(numSectors, defaultAzimuths) {
    ...
    for (let i = 0; i < numSectors; i++) {
      ...
      inp.type = 'number'; inp.id = `az_sec_${i}`;
      inp.min  = '0'; inp.max = '359'; inp.step = '1';
      inp.value = defAz; inp.title = `Azimuth Sektor ${i+1}`;
  ...
  function syncRFFromShapefile() {
    ...
    const setVal = (id, val) => {
      ...
      for (const opt of el.options) if (opt.value === String(val)) { el.value = String(val); break; }
    };
    if (s.scenario)  setVal('rf_scenario',  s.scenario);
    if (s.condition) setVal('rf_condition', s.condition);
    CAL.CLUTTER = s.clutter ? s.clutter.toLowerCase().replace(/[\s-]+/g,'_') : 'n/a';
  ...
  function rebuildSectorsFromRF(rfParams) {
    ...
    for (let i = 0; i < rfParams.numSectors; i++) {
      ...
      newServingSectors.push({
        siteId: driveTestData.siteId, siteLat: s.lat, siteLng: s.lng,
        siteHeight: rfParams.antHeight, isMain: true, nbIdx: -1,
        sectorNum: i+1, azimuth: az,
        pci: origSec.pci ?? null, cellId: origSec.cellId ?? null,
        cellName: origSec.cellName ?? `${driveTestData.siteId}_Sek${i+1}`,
        gnbId: origSec.gnbId ?? s.gnbId ?? null, arfcn: origSec.arfcn ?? 466850,
        pciColor: MAIN_SECTOR_COLORS[i % MAIN_SECTOR_COLORS.length],
        scenario: rfParams.scenario, condition: rfParams.condition, clutter: rfParams.clutter,
      });
    }
    ...
    Object.values(siteMap).forEach(site => {
      ...
      site.sectors.forEach((sec, i) => {
        drawSectorFan(site.lat, site.lng, sec.azimuth, CAL.BEAMWIDTH,
          site.isMain ? 80 : 90, i, site.isMain ? 0.18 : 0.07, sec.pciColor);
      });
    });
  }
  ...
  function initRFPanel() {
    ...
    ['rf_txpower','rf_frequency','rf_bandwidth','rf_scenario','rf_condition',
     'rf_ant_height','rf_beamwidth'].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
    ...
    if (numSecEl) {
      numSecEl.addEventListener('change', () => {
        ...
        initAzimuthInputs(n, Array.from({length:n},(_,i) => Math.round(i*(360/n))));
        updateRFBadges(); autoRecalcOnRFChange();
      });
    }
    updateRFBadges();
  }
  ...
  function autoRecalcOnRFChange() {
    ...
    autoRecalcTimer = setTimeout(() => {
      try {
        if (calcState === 'sinr') { _doCalculateRSRP(true); _doCalculateSINR(true); }
        else { _doCalculateRSRP(true); }
        setRecalcIndicator('done');
      } catch(e) { console.error('[autoRecalc]', e); setRecalcIndicator('idle'); }
  ...
  function buildAllSectors() {
    ...
    const servSectors = servSectorData.length > 0
      ...
      : (s.sectors || []).map((az, i) => ({
          ...
          cellName: `${driveTestData.siteId}_Sek${i+1}`, gnbId: null, arfcn: 466850,
        }));
    servSectors.forEach((sec, i) => {
      allSectors.push({
        siteId: driveTestData.siteId, siteLat: s.lat, siteLng: s.lng,
        siteHeight: s.height || 30, isMain: true, nbIdx: -1,
        ...
        gnbId: sec.gnbId || s.gnbId || null, arfcn: sec.arfcn || 466850,
        pciColor: MAIN_SECTOR_COLORS[i % MAIN_SECTOR_COLORS.length],
        scenario: s.scenario || 'uma', condition: s.condition || 'nlos', clutter: s.clutter || 'N/A',
      });
    });
    (driveTestData.neighbours || []).forEach((nb, nbIdx) => {
      const nbSectors = (nb.sectorData || []).length > 0
        ...
        : (nb.sectors || []).map((az, i) => ({
            sectorNum: i+1, azimuth: az, pci: null, cellId: null,
            cellName: `${nb.siteId}_Sek${i+1}`, gnbId: null, arfcn: 466850,
          }));
      nbSectors.forEach((sec, secIdx) => {
        allSectors.push({
          siteId: nb.siteId, siteLat: nb.lat, siteLng: nb.lng,
          siteHeight: nb.height || 30, isMain: false, nbIdx,
          sectorNum: sec.sectorNum || (secIdx+1), azimuth: sec.azimuth,
          pci: sec.pci, cellId: sec.cellId,
          cellName: sec.cellName || `${nb.siteId}_Sek${sec.sectorNum||secIdx+1}`,
          gnbId: sec.gnbId || nb.gnbId || null, arfcn: sec.arfcn || 466850,
          pciColor: NEIGHBOUR_PALETTE[(nbIdx*6+secIdx) % NEIGHBOUR_PALETTE.length],
  ...
  function generateSamplingPoints() {
    ...
    if (USE_JOURNAL_SNAPSHOT && JOURNAL_SNAPSHOT.length > 0) {
      ...
      console.log(`[Sampling] 📖 Journal snapshot: ${samplingPoints.length} titik`);
  ...
  function _verifySamplingInterval() {
    ...
    console.log(`[SamplingVerify] n=${samplingPoints.length} | min=${minD.toFixed(2)}m | max=${maxD.toFixed(2)}m | avg=${avg.toFixed(2)}m`);
  ...
  function _doCalculateRSRP(silent) {
    ...
    samplingPoints.forEach((point, idx) => {
      ...
      cellResults[0].category = 'serving';
      cellResults.slice(1).forEach(c => {
        c.category = c.rsrp >= RSRP_DETECTED_THRESH ? 'detected' : 'listed';
      });

      const marker = L.circleMarker([point.lat, point.lng], {
        radius:5, fillColor:rsrpColor(cellResults[0].rsrp),
        color:'#000', weight:1, fillOpacity:0.9,
      }).addTo(simHeatmapLayer);
      marker.on('click', () => onPointClick(point, idx+1, cellResults));
      rsrpResults.push({ index:idx+1, lat:point.lat, lng:point.lng, cells:cellResults, serving:cellResults[0] });
    });
  ...
  function _doCalculateSINR(silent) {
    ...
    rsrpResults.forEach((result, idx) => {
      ...
      result.cells.forEach((c, ci) => {
        ...
        const sinr_nb = Math.max(-40, Math.min(40, linearToDbm(S_nb / (I_nb + N_linear))));
        c.sinr     = sinr_nb;
    ...
    rsrpResults.forEach((result, idx) => {
      const marker = L.circleMarker([result.lat, result.lng], {
        radius:5, fillColor:sinrColor(result.sinr),
        color:'#000', weight:1, fillOpacity:0.9,
      }).addTo(simHeatmapLayer);
    ...
    if (!silent) {
      ...
      const thermalDbm = getThermalNoise();
      // Distribusi selaras dengan legend 5 bucket
      ...
      const pn5  = ((rsrpResults.filter(r => r.sinr >= -5 && r.sinr < 0 ).length / total) * 100).toFixed(1);
      const pn40 = ((rsrpResults.filter(r => r.sinr < -5).length / total) * 100).toFixed(1);
      alert(
        `Kalkulasi SINR selesai\n` +
        `Formula serving  : SINR = S / (I + N + I_IM)\n` +
        `Formula neighbour: SINR_nb = S_nb / (I_partial + N_eff)\n` +
        `Thermal noise N  : ${thermalDbm.toFixed(1)} dBm  [= -174 + 10·log(BW) + NF]\n` +
  ...
  function initCellPanelPills() {
    ...
    pills.innerHTML = labels.map((l,i) =>
      `<span class="line-pill" style="background:${colors[i]}">${l}</span>`).join('');
  }
  ...
  function updateCellTable(point, ptIdx, cells) {
    ...
    // ── [FIX-L4] dotColor selaras dengan 5-bucket legend ─────────────────
    const dotColor = (val, type) => {
      ...
      if (type === 'rsrp') {
        ...
        return '#ff3333'; // -140 ~ -120 dan di bawahnya
      }
      if (type === 'sinr') {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // [FIX-L1 & FIX-L2] LEGEND — 5 bucket, label sesuai spesifikasi
  // RSRP: -85~0 | -95~-85 | -105~-95 | -120~-105 | -140~-120
  // SINR: 20~40 | 10~20   | 0~10     | -5~0      | -40~-5
  // ══════════════════════════════════════════════════════════════════════════
  function updateLegend(type) {
    ...
    const buckets = type === 'RSRP' ? [
      { label:'-85 ~ 0',      color:'#0042a5', fn: v => v >= -85                  },
  ...
  window.getJournalSnapshot = function () {
    if (!samplingPoints.length) { console.warn('Belum ada titik sampling. Generate dulu!'); return null; }
  ...
  window.getSimulationSummary = function () {
    ...
    const total   = rsrpResults.length;
    const hasSINR = rsrpResults[0]?.sinr !== undefined;
    console.log(`=== SIMULATION SUMMARY v4.5.1 | Seed: ${FIXED_SEED} ===`);
    console.log(`Total titik: ${total}`);
    console.log(`Avg RSRP: ${(rsrpResults.reduce((s,r)=>s+r.serving.rsrp,0)/total).toFixed(2)} dBm`);
    if (hasSINR)
      console.log(`Avg SS-SINR: ${(rsrpResults.reduce((s,r)=>s+r.sinr,0)/total).toFixed(2)} dB`);
  };
